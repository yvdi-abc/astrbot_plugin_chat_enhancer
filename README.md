...
[![Version](https://img.shields.io/badge/version-v1.1.1-blue.svg)](https://github.com/yvdi-abc/astrbot_plugin_chat_enhancer)
...
1. **消息智能分段** - 将长消息分段发送，模拟真实聊天节奏，支持单段长度控制
2. **Markdown 格式消除** - 自动移除 `**粗体**`、`*斜体*`、`` `代码` `` 等格式（含表格、标题、引用等）
3. **智能合并转发** - 检测长篇回答，自动使用聊天记录形式发送（已修复合并转发失效问题）
4. **可选开关** - 所有功能都可以独立开关控制
5. **平台自适应** - 非 QQ 平台自动降级为分段发送，不会尝试无效的合并转发
6. **合并转发测试** - `/增强测试` 指令一键验证合并转发是否正常
...
| `min_segment_len` | int | `10` | 最小分段长度（字符），避免碎片化短段 |
| `max_segment_len` | int | `200` | 单段最大长度（字符），超长段落自动二次切分 |
...
1. 消息长度 > `forward_threshold`
2. 分段数 > `max_segments`
3. 用户消息包含触发关键词

**合并转发仅在 aiocqhttp / qq_official 平台生效**，其他平台自动降级为分段发送。
...
### v1.1.1
- 🐛 **修复插件命令（如 /ask）回复不触发合并转发的问题**：`on_decorating_result` 不再依赖 `on_llm_response` 标记，改为直接从消息链内容独立判断，主对话与插件回复均可合并转发
- ✨ 检测到消息链已有合并转发组件时自动跳过，避免与其他插件冲突

### v1.1.0
- 🐛 **修复合并转发失效问题**：`event.self_id` 属性不存在导致合并转发必然失败，改为 `event.get_self_id()`，合并转发现在可以正常工作
- 🐛 修复分段数判断逻辑：原先分段被提前合并导致"分段数超限"判断永远不成立
- 🐛 修复 Markdown 标题消除在行中间不生效的问题
- ✨ 新增 `min_segment_len` / `max_segment_len` 配置项，控制分段粒度
- ✨ 新增 `/增强测试` 指令，一键验证合并转发功能
- ✨ 平台自适应：非 QQ 平台自动降级为分段发送，不再无效尝试合并转发
- ✨ 同步发送替代异步 create_task，避免消息丢失时序问题
- ✨ 合并转发节点数超过 50 时自动合并，避免单条转发消息超限
